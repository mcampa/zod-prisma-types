...
describe('writeSingleFileHeading', () => {
  ...
  it('should write blank lines between statements', () => {
    // Execute
    writeSingleFileHeading(mockFileOptions);

    // Verify blank lines were written
    expect(blankLineCalls).toBe(3);
  });
  ...
  it('should call writer methods in the correct order', () => {
    ...
    expect(mockFileOptions.writer.blankLine).toHaveBeenCalledTimes(3);

    // Verify order: writeLine, blankLine, writeLine, blankLine, writeLine, blankLine
    const calls = vi.mocked(mockFileOptions.writer.writeLine).mock
      .invocationCallOrder;
    const blankCalls = vi.mocked(mockFileOptions.writer.blankLine).mock
      .invocationCallOrder;

    expect(calls[0]).toBeLessThan(blankCalls[0]);
    expect(blankCalls[0]).toBeLessThan(calls[1]);
    expect(calls[1]).toBeLessThan(blankCalls[1]);
    expect(blankCalls[1]).toBeLessThan(calls[2]);
    expect(calls[2]).toBeLessThan(blankCalls[2]);
  });
